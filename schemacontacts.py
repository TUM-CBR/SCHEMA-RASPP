...
def main_impl(arg_dict):
        ...
        with open(pdb_file, 'r') as res:
                residues = pdb.File().read(res)
        ...
        if not parent_pdb_alignment_file:  # Just get PDB sequence from the multiple sequence alignment
                ...
                with open(parent_pdb_alignment_file, 'r') as res:
                        pdb_parent_seq_list = schema.readMultipleSequenceAlignmentFile(res)
